      ...
      if(paste0(ChannelName[i],"_", AttributionModel) %in% ls_RegUsingLags){
                      revPred_allchannel = c( revPred_allchannel,                        
                                  ...
                                  if(SkipMonths==0){
                                                    ...
                                                    print("fc_a_4")
                                                      ...
                                                      NegativeChannelRevCalculation<-function(E){
                                                                ...
                                                                for(q in (nrow(z)-planMonths+1):nrow(z)){
                                                                              ...
                                                                              z[q,paste0(Channel,"_l12")]<-z[(q-11),Channel]
                                                                              z[q,paste0(Channel,"_l13")]<-z[(q-12),Channel]
                                                                     }
                                                                z <- z[(nrow(z)-planMonths+1):nrow(z),] 
                                       ...
                                       }else{ #skipMonths>0 ==> means, it's Planforward
                                          print("fc_a_7")
                                          ...
                                          if(class(SpendingForSkippingMonths)=="try-error"){
                                                      ...
                                                      for(q in 1:SkipMonths){
                                                              ...
                                                              data_temp[nrow(data_temp),"X12"]<-data_temp[(nrow(data_temp)-11),"X"]
                                                              data_temp[nrow(data_temp),"X13"]<-data_temp[(nrow(data_temp)-12),"X"]
                                          ...
                                          NegativeChannelRevCalculation<-function(x){
                                                    ChannelExp_ModelingSet<- data.frame("ChannelExp_ModelingSet"=log(c(SpendingForSkippingMonths,x)))
                                                    predSet <- forecast( fit, h=SkipMonths+planMonths
                                                                         , xreg= ChannelExp_ModelingSet)$mean[(SkipMonths+1):(SkipMonths+planMonths)]
                                                    return(-sum(exp(predSet)))
                                                    }
...
run2ProjROI  <- AR[length(AR)]/AS[length(AS)] - 1
run2ProjROI  <- paste0(round(100*run2ProjROI, 0), "%")

print(paste0("Total AR: $", format(AR[length(AR)],  big.mark=",", big.interval=3)))
print(paste0("Total AS: $", format(AS[length(AS)],  big.mark=",", big.interval=3)))
print(paste0("Projected ROI: ", run2ProjROI))
...
AlgDuration=format(AlgDuration)
  ...
  PercentageChangeInTotalRev <- round((OptTotalRevenue-as.numeric(inJsonData$OptTotalRevenue))/as.numeric(inJsonData$OptTotalRevenue)*100, roDgt)
  }
...
commafmt <- function(x)  format(x,  big.mark=",", big.interval=3)
  ...
  #cat(paste0('"OptTotalRevenue": ', '"', inJsonData$OptTotalRevenue, '",\n'))## added for optimizedTotalRevenue  for Run_1    
  cat(paste0('"OptTotalRevenue": ', '"',inJsonData$OptTotalRevenue, '",\n'))## added for optimizedTotalRevenue  for Run_1    
  cat(paste0('"', paste0(PRChannel,  'PR' ),  '": "', inJsonData[ paste0(PRChannel,  'PR' )], '",\n'))

  cat(paste0('"run1RevRange":',    '"', inJsonData$run1RevRange,  '",\n'))
  # After Run2, run1ProjROI & run2ProjROI both has the value of run2ProjROI.
  cat(paste0('"run1ProjROI":',     '"', run2ProjROI,   '",\n'))
  cat(paste0('"run1ROIRange":',    '"', inJsonData$run1ROIRange,  '",\n'))
  
  
  cat(paste0('"', paste0(ChannelOut,    'SlideLeft'   ),  '": "', inJsonData[ paste0(ChannelOut, 'LB' )], '",\n'))
  cat(paste0('"', paste0(ChannelOut,    'Slide'       ),  '": "', inJsonData[ paste0(ChannelOut,    'Slide'       )], '",\n'))
  ...
  cat(paste0('"', paste0(ASChannel,     'AS'          ),  '": "', round(AS, roDgt), '",\n'))
  
  #cat(paste0('"OptTotalRevenue_Run2": ', '"', round(OptTotalRevenue, roDgt), '",\n'))## added for optimizedTotalRevenue  for Run_2   
  cat(paste0('"OptTotalRevenue_Run2": ', '"', round(OptTotalRevenue, roDgt), '",\n'))## added for optimizedTotalRevenue  for Run_2   
  cat(paste0('"PercentageChangeInTotalRev": ', '"', PercentageChangeInTotalRev, '",\n'))  
  cat(paste0('"', paste0(ARChannel,     'AR'          ),  '": "', round(AR, roDgt), '",\n'))
