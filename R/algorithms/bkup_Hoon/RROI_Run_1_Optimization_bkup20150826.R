...
# ModelV2. added
if(nrow(OneWeekPrior)==0){OneWeekPrior[1:7,]<-rep(1, ncol(OneWeekPrior)) }
...
if(IncludeDataForThePeriod=="NO" | SkipMonths==0){Estimate = as.list(Estimate[Estimate$Beg_Date==as.Date(Beg_Date),])}else{
    if(SkipMonths>0){
              Estimate = as.list(Estimate[nrow(Estimate),]) #take the latest coef. valeus. SkipMonths>0 means Planforward.                        
    }else{  # case for IncludeDataForThePeriod=="YES"
              Estimate = as.list(Estimate[Estimate$Beg_Date==as.Date(End_Date)+1,]) #End_date is the last DATE for projection. Coef. using the whole history up to the End_Date shall be used. 
    }
}
#########################################################################
##### End: ModevlV2
